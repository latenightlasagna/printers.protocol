...
self.onmessage = function(e) {
    ...
    for (let i = 0; i < grid.length; i++) DT1[i] = grid[i] === 1 ? Infinity : 0;
    ...
    // FIX: Brightness tolerance raised to 60 to safely ignore CMYK rich blacks.
    // Edge bypass increased to >3 to completely jump over heavy anti-aliasing.
    for (let i = 0; i < grid.length; i++) {
        if (grid[i] === 1 && DT1[i] > 3) {
            const brightness = (data[i*4] + data[i*4+1] + data[i*4+2]) / 3;
            if (brightness > 60 && brightness < 235) {
    ...
    for (let i = 0; i < DT1.length; i++) DT2[i] = (DT1[i] >= radiusThreshold) ? 0 : Infinity;

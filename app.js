document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateStrokeInfo() {
        ...
        if (opt.value === 'custom') customStrokeGroup.classList.remove('hidden');
        else customStrokeGroup.classList.add('hidden');
        
        strokeInfo.innerText = `(Min: ${min || 0} mm)`;
    }
    ...
    document.getElementById('close-modal').addEventListener('click', () => {
        ...
        toggleMeasureBtn.innerText = "Interactive Measure: OFF";
        toggleMeasureBtn.style.color = "var(--text-color)";
        measureHint.classList.add('hidden');
    });

    // --- 3. INTERACTIVE TOOL (IN MODAL ONLY) ---
    toggleMeasureBtn.addEventListener('click', () => {
        isMeasuringMode = !isMeasuringMode;
        if(isMeasuringMode) {
            toggleMeasureBtn.innerText = "Interactive Measure: ON";
            toggleMeasureBtn.style.color = "var(--blue)";
            actualSizeContainer.classList.add('measuring');
    ...
    interactCanvas.addEventListener('mousedown', (e) => {
        ...
        else { p2 = applyShiftConstraint(coords, e.shiftKey); isDrawingLine = false; renderLine(p1, p2, true); }
    });

    interactCanvas.addEventListener('mousemove', (e) => {
        if(!isMeasuringMode || !isDrawingLine) return;
        renderLine(p1, applyShiftConstraint(getMousePos(e), e.shiftKey), false);
    });
    ...
    function renderLine(start, end, isFinal) {
        interactCtx.clearRect(0,0, interactCanvas.width, interactCanvas.height);
        ...
        const tx = end.x + 15; const ty = end.y + 15;
